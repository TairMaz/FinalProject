...
## 4. wsptFlowshop.cpp
קובץ בשפת c++ המכיל אלגוריתם למציאת הסידור האופטימלי בזמן ריצה של o(n^2)
האלגוריתם קודם על ממין את סדר העבודות לפי היחס בין הזמן עיבוד למשקל (מהגבוה לנמוך)
לאחר מכן עובר על כל תתי הקבוצות האפשריות של איבר מסוים ומכניס את אותה עבודה במיקום הטוב ביותר.
הבדיקה היכן להכניס כל עבודה בסדור הנוכחי עד שהגענו עליה עובד בלולאה על כל האיברים עד האיבר הנוכחי אבל שומרים לפני כן את הערך המקסימלי בכל מיקום ואת הזמן עיבוד בכל מיקום. כך שבמעבר על כל תת קבוצה נבצע את הבדיקה בזמן של o(1) מה שגורם לזמן ריצה להיות מינימלי.
האלגוריתם מתבצע תוך כדי השווואה לאלגוריתם הנאיבי על מנת לבדוק גם נכונות.

## 5. wsptFlowshop_DP.cpp

קובץ זה מממש אלגוריתם מבוסס תכנות דינמי (DP) לפתרון בעיית תזמון עבודות במודל Flowshop עם אפשרות לדחיית עבודות תחת מגבלת תקציב (U). האלגוריתם משלב מיון ראשוני לפי יחס משקל לזמן עיבוד (WSPT), ולאחר מכן בונה פתרון אופטימלי באופן הדרגתי תוך החלטה עבור כל עבודה האם לשבץ אותה או לדחות אותה. עבור עבודות שנבחרו לשיבוץ, מתבצעת הכנסתן למיקום הטוב ביותר בסידור הקיים על בסיס חישוב אינקרמנטלי של פונקציית המטרה (MCI), המאפשר חישוב יעיל ללא צורך בחישוב מלא מחדש. האלגוריתם שומר עבור כל תת־בעיה את העלות המינימלית שניתן להשיג תחת מגבלת התקציב, ובסיום משחזר את הסידור האופטימלי ואת קבוצת העבודות שנדחו. סיבוכיות הזמן של האלגוריתם היא O(n^2 * U), והוא מאומת באמצעות השוואה לאלגוריתם נאיבי הבודק את כל האפשרויות.
